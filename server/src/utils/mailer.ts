...
export const sendWelcomeMail = async (user: {
  ...
  const text = [
    `Dear ${user.name},`,
    '',
    `Welcome to QuizPulse. An account has been created for you with the role of ${role}.`,
    '',
    'Your login details:',
    `  Email: ${user.email}`,
...
<html><body style="margin:0;padding:24px;background:#f4f5f7;font-family:Arial,Helvetica,sans-serif;color:#1f2937">
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;margin:0 auto;background:#ffffff;border-radius:12px;border:1px solid #e5e7eb">
    <tr><td style="padding:28px 32px;border-bottom:1px solid #e5e7eb">
      <div style="font-size:20px;font-weight:bold;color:#0E8A7D">QuizPulse</div>
    </td></tr>
    <tr><td style="padding:28px 32px;font-size:15px;line-height:1.6">
      <p style="margin:0 0 16px">Dear ${e.name},</p>
      <p style="margin:0 0 16px">Welcome to QuizPulse. An account has been created for you with the role of <strong>${e.role}</strong>.</p>
      ...
      <p style="margin:0 0 24px"><a href="${e.loginUrl}" style="display:inline-block;background:#0E8A7D;color:#ffffff;text-decoration:none;padding:12px 24px;border-radius:8px;font-weight:bold">Sign in to QuizPulse</a></p>
      ...
      <p style="margin:0">Regards,<br>Team QuizPulse</p>
    </td></tr>
  </table>
</body></html>`;

  try {
    await sendMail({ to: user.email, subject: 'Your QuizPulse account details', text, html });
  } catch (error) {
    console.error(
      JSON.stringify({
        level: 'error',
